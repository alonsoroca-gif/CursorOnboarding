...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    "cursorSetup.startOnboardingChat",
    async () => {
      ...
      const choice = await vscode.window.showInformationMessage(
        "Cursor Setup (Part 2): If Chat didn't open with a prompt, paste the starter below into Chat (Cmd+Shift+L or Ctrl+Shift+L).",
        "Copy starter prompt",
        "Dismiss"
      );
      ...
      if (choice === "Copy starter prompt") {
        ...
        vscode.window.showInformationMessage(
          "Starter prompt copied. Open Chat (Cmd+Shift+L / Ctrl+Shift+L) and paste it to start Part 2."
        );
      }
